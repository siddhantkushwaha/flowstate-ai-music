...
- `history.py`: `GET/POST /history`, `DELETE /history/<id>` — per-user history of played curated sessions, 7-day lazy retention (`Config.HISTORY_RETENTION_SECONDS`). `POST` is an upsert keyed by (user, normalized prompt) — the caller always sends the session's full current state (tracks, steer_history), so it's safe to call on every queue modification, not just the first save. Known tradeoff: two parallel sessions on the same account (e.g. phone + laptop) curating the identical prompt at the same time will upsert into the same row — accepted since it's the same user's own data.
...
- `App.jsx` saves a curated session to history once its first track has played continuously for 30s (`HISTORY_SAVE_THRESHOLD_MS`) - filters out prompts that were typed but never actually listened to. After that, `syncSessionToHistory` re-upserts the same entry on every subsequent queue change (steering, Infinite Flow additions, track removal) via an effect watching `[queue, curatorSummary, steerHistory]`, gated by `historyEligibleRef` - so any queue modification is reflected, not just explicit steers.
